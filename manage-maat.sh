...
mode="${1:-}"
if [[ -z "$mode" ]]; then
  echo "Usage: $0 {install|check|doctor|uninstall|start|stop|restart|logs|list-projects|set-project|new-project|build-runners|init-config}" >&2
  exit 2
fi
...
stop_maat_docker_containers() {
  if command -v docker >/dev/null 2>&1; then
    local ids ids_by_name ids_by_label ids_by_image lang_dir profile image
    ids_by_name="$(docker_cmd ps -aq --filter 'name=^/maat_' 2>/dev/null || true)"
    ids_by_label="$(docker_cmd ps -aq --filter 'label=maat.managed=true' 2>/dev/null || true)"
    ids_by_image=""
    shopt -s nullglob
    for lang_dir in "$ROOT_DIR"/languages/*; do
      [[ -d "$lang_dir" && -f "$lang_dir/language.json" ]] || continue
      profile="$lang_dir/language.json"
      image="$(PYTHONPATH="$ROOT_DIR:$VENDOR_DIR:${PYTHONPATH:-}" "$PYTHON_BIN" - "$profile" <<'PY_DOCKER_IMAGE'
import json, sys
try:
    print(json.load(open(sys.argv[1], encoding='utf-8')).get('docker_image',''))
except Exception:
    print('')
PY_DOCKER_IMAGE
)"
      [[ -n "$image" ]] || continue
      ids_by_image+=$'
'"$(docker_cmd ps -aq --filter "ancestor=$image" 2>/dev/null || true)"
    done
    shopt -u nullglob
    ids="$(printf '%s
%s
%s
' "$ids_by_name" "$ids_by_label" "$ids_by_image" | awk 'NF && !seen[$0]++')"
    if [[ -n "$ids" ]]; then
...
local_admin = f"http://127.0.0.1:{cfg.get('listen_port', 8000)}/admin"
public_url = str(cfg.get('public_url') or '').rstrip('/')
public_admin = f"{public_url}/admin" if public_url else ''
admin_token = str(cfg.get('admin_token') or '')
print(f"Admin local     : {local_admin}")
if public_admin and not public_admin.startswith('http://localhost') and '127.0.0.1' not in public_admin:
    print(f"Admin public    : {public_admin}")
print(f"Admin token     : {admin_token if admin_token else 'not configured'}")
print('')
