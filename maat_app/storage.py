...
class Storage:
    ...
    def validate_submission_allowed(self, token: str) -> tuple[bool, str, int, str]:
        ...
        for status in self.list_statuses():
            ...
            if status.get("status") in active:
                return False, "already_active", 0, "A submission is already queued or running for this token."
        ...
        if latest_ts is not None:
            ...
            if unlock_ts < latest_ts:
                ...
                if remaining > 0:
                    return False, "cooldown", remaining, "You must wait before submitting again."
        return True, "", 0, ""
    ...
    def secure_save_zip(self, uploaded: FileStorage, submission_id: str, language_id: str | None = None) -> Path:
        ...
        if path.stat().st_size > max_bytes:
            ...
            raise ValueError(f"Archive too large: limit {self.cfg.get('max_zip_size_mb')} MB.")
        try:
    ...
    def validate_zip_content(self, zip_path: Path, language_id: str | None = None) -> None:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            if len(infos) > max_files:
                raise ValueError(f"Archive rejected: more than {max_files} files.")
            for info in infos:
                name = info.filename.replace("\\", "/")
                lower_name = name.lower().lstrip("/")
                if name.startswith("/") or ".." in Path(name).parts:
                    raise ValueError("Archive rejected: unsafe path detected.")
                mode = (info.external_attr >> 16) & 0o170000
                permissions = (info.external_attr >> 16) & 0o777
                if mode == 0o120000:
                    raise ValueError("Archive rejected: symbolic links are not allowed.")
                if any(lower_name.startswith(prefix) for prefix in ignored_prefixes):
                    continue
                total += info.file_size
                if total > max_uncompressed:
                    raise ValueError(f"Archive rejected: uncompressed content exceeds {self.cfg.get('max_uncompressed_size_mb')} MB.")
                if not info.is_dir():
                    ...
                    if permissions & 0o111 and not is_allowed_text_candidate:
                        raise ValueError(f"Archive rejected: forbidden executable file: {name}.")
                    if not is_allowed_text_candidate:
                        raise ValueError(f"Extension not allowed in archive: {ext or '(none)'}")
                    with zf.open(info, "r") as handle:
                        sample = handle.read(min(info.file_size, 1024 * 1024))
                    if b"\0" in sample:
                        raise ValueError(f"Archive rejected: forbidden binary file: {name}.")
    ...
    def cancel_submission(self, submission_id: str) -> dict[str, Any] | None:
        ...
        status["message"] = "Submission canceled/withdrawn."
