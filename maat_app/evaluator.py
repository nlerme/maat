...
class Evaluator:
    ...
    def evaluate(self, submission_id: str) -> None:
        ...
        if not status or self.storage.is_canceled(submission_id):
            return
        try:
            status.update({"status": "running", "started_at": now_iso(), "message": "Évaluation en cours.", "message_key": "running_msg", "message_args": {}})
            ...
            if not compile_ok or self.storage.is_canceled(submission_id):
                return
            self.run_instances(run_dir, project_dir, status, profile)
            ...
            if status.get("status") == "canceled":
                ...
                self.storage.save_status(submission_id, status)
                return
            self.finish_with_error(submission_id, status, "internal_error", f"Erreur interne : {exc}", "internal_error_msg", {"error": str(exc)})
    ...
    def language_profile(self, language_id: str) -> dict[str, Any]:
        ...
        if not isinstance(profile, dict):
            raise InvalidArchiveError(f"Langage non autorisé pour ce projet : {language_id}.")
        return profile
    ...
    def prepare_project(self, run_dir: Path, zip_path: Path, language_id: str) -> Path:
        ...
        try:
            ...
            raise InvalidArchiveError("Archive ZIP invalide ou corrompue.") from exc
        ...
        with zf_context as zf:
            ...
            for info in zf.infolist():
                ...
                if name.startswith("/") or ".." in Path(name).parts:
                    raise InvalidArchiveError("Chemin non sûr dans l'archive.")
                mode = (info.external_attr >> 16) & 0o170000
                if mode == 0o120000:
                    raise InvalidArchiveError("Archive refusée : liens symboliques interdits.")
    ...
    def find_project_root(self, extract_dir: Path, profile: dict[str, Any]) -> Path:
        ...
        if not candidates:
            ...
            raise MissingExpectedFileError(f"Fichier attendu manquant : aucun point d'entrée trouvé ({expected}).")
    ...
    def check_forbidden_patterns(self, project_dir: Path, profile: dict[str, Any]) -> None:
        ...
        for path in project_dir.rglob("*"):
            ...
            if b"\0" in data:
                ...
                raise ForbiddenPatternError(f"Dépôt refusé : fichier binaire interdit détecté : {rel}.")
            text = data.decode("utf-8", errors="replace")
            for name, pattern in patterns:
                if pattern.search(text):
                    rel = path.relative_to(project_dir).as_posix()
                    raise ForbiddenPatternError(f"Dépôt refusé : motif interdit détecté : {name} dans {rel}.")
    ...
    def compile_project(self, run_dir: Path, project_dir: Path, status: dict[str, Any], profile: dict[str, Any]) -> bool:
        ...
        if not command:
            status.update({"compile_runtime_seconds": 0.0, "message": "Aucune compilation nécessaire.", "message_key": "compile_ok_msg", "message_args": {}})
            self.storage.save_status(submission_id, status)
            return True
        ...
        if timed_out:
            status.update({"status": "compile_timeout", "finished_at": now_iso(), "message": "Temps limite dépassé pendant la compilation.", "message_key": "compile_timeout_msg", "message_args": {}})
            self.storage.save_status(submission_id, status)
            return False
        if returncode != 0:
            ...
            if returncode in {137, 143}:
                ...
                message = "Erreur de compilation : limite mémoire/ressource Docker probablement atteinte."
            status.update({"status": "compile_error", "finished_at": now_iso(), "message": message, "message_key": message_key, "message_args": {}})
            self.storage.save_status(submission_id, status)
            return False
        status.update({"message": "Compilation réussie.", "message_key": "compile_ok_msg", "message_args": {}})
        self.storage.save_status(submission_id, status)
        return True

    def run_instances(self, run_dir: Path, project_dir: Path, status: dict[str, Any], profile: dict[str, Any]) -> None:
        submission_id = status["submission_id"]
        ...
        for index, instance_path in enumerate(instances, start=1):
            ...
            self.storage.save_status(submission_id, status)
            result = self.run_one_instance(run_dir, project_dir, submission_id, instance_path, profile)
            latest = self.storage.get_status(submission_id) or status
            status = latest
        ...
        status["message"] = "Évaluation terminée."
    ...
    def run_one_instance(self, run_dir: Path, project_dir: Path, submission_id: str, instance_path: Path, profile: dict[str, Any]) -> dict[str, Any]:
        instance_safe = safe_name(instance_path.stem)
        ...
        returncode, timed_out = self.runner.run(
            ...
            cancel_check=lambda: self.storage.is_canceled(submission_id),
            extra_readonly_mounts=[(Path(self.cfg["data_dir_abs"]), "/judging/data")],
        )
        elapsed = round(time.perf_counter() - t0, 3)
        ...
        if self.storage.is_canceled(submission_id):
            result.update({"status": "CANCELED", "message": "Annulé."})
            return result
        if timed_out or returncode == 124:
            result.update({"status": "TIMEOUT", "message": "Temps limite dépassé."})
            return result
        if returncode != 0:
            result.update({"status": "RUNTIME_ERROR", "message": f"Code retour {returncode}."})
            return result
        ...
        if parsed is None:
            result.update({"status": "INVALID_OUTPUT", "message": "Format stdout invalide."})
            return result
    ...
    def list_instances(self) -> list[Path]:
        data_dir = Path(self.cfg["data_dir_abs"])
        pattern = str(self.cfg.get("public_instances_glob", "instance_*"))
        return sorted(data_dir.glob(pattern), key=lambda p: self.natural_key(p.name))
